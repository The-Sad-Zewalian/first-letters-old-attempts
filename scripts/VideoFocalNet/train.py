...
from videofocalnet import VideoFocalNet
...
class CFG:
    ...
    fragments =     ["S5_20241030152030",       "S5_20241108111522",       "S5_20241102125650",       "S1_20230929220920",       "S4_20231210132040",       "S1_20240110113230",         "S3_20240618142020",       "S3_20240716140050",       "S3_20240712074250",       "S1_20231004222109",         'S1_20230515162442',         '20230517214715',       '20230422011040',       '20230422213203',       '20230426114804',       '20230522182853',       '20230709155141',       '20230801194757',          '20240516205750',   '20230509173534']
    start_idxs =    {"S5_20241030152030":17,    "S5_20241108111522":17,    "S5_20241102125650":17,    "S1_20230929220920":17,    "S4_20231210132040":17,    "S1_20240110113230":39,      "S3_20240618142020":17,    "S3_20240716140050":17,    "S3_20240712074250":17,    "S1_20231004222109":17,      'S1_20230515162442':17,      '20230517214715':17,    '20230422011040':17,    '20230422213203':17,    '20230426114804':17,    '20230522182853':17,    '20230709155141':17,    '20230801194757':17,    '20240516205750':17,    '20230509173534':17}
    reversed_load = {"S5_20241030152030":False, "S5_20241108111522":False, "S5_20241102125650":False, "S1_20230929220920":False, "S4_20231210132040":False, "S1_20240110113230":False,   "S3_20240618142020":False, "S3_20240716140050":False, "S3_20240712074250":False, "S1_20231004222109":False,   'S1_20230515162442':False,   '20230517214715':False, '20230422011040':False, '20230422213203':False, '20230426114804':False, '20230522182853':False, '20230709155141':False, '20230801194757':False, '20240516205750':False, '20230509173534':False}
    flipped =       {"S5_20241030152030":False, "S5_20241108111522":False, "S5_20241102125650":False, "S1_20230929220920":False, "S4_20231210132040":False, "S1_20240110113230":False,   "S3_20240618142020":False, "S3_20240716140050":False, "S3_20240712074250":False, "S1_20231004222109":False,   'S1_20230515162442':False,   '20230517214715':False, '20230422011040':False, '20230422213203':False, '20230426114804':False, '20230522182853':False, '20230709155141':False, '20230801194757':False, '20240516205750':False,' 20230509173534':False}

    # ================== model cfg =================
    ckpt = "/content/drive/MyDrive/Scrolls/ckpts/Vesuvius_S2_T21/Training_S2_T21_E6/Vesuvius_S2_T21-models/VideoFocalNet_T21_E6_size=64_S1_20230515162442_epoch=6.ckpt"
    backbone = 'VideoFocalNet'
    in_chans = 26
    load_chans = 33
    avg_n = 0
    scale_factor = 1
    ...
    train_aug_list = A.Compose(transforms=[
        A.Resize(size, size),
        A.HorizontalFlip(p=0.5),
        A.VerticalFlip(p=0.5),
        A.ShiftScaleRotate(rotate_limit=360, shift_limit=0.15, scale_limit=0.15, p=0.5),
        A.OneOf([
                ...
                p=0.5),
        A.CoarseDropout(
                  num_holes_range=(4, 64),
                  hole_width_range=(0.0625, 0.25),
                  hole_height_range=(0.0625, 0.25), 
                  mask_fill_value=0,
                  p=0.5),
        A.OneOf([
                  A.Morphological(scale=(1, int(size * 0.125)), operation='dilation'),
                  A.Morphological(scale=(1, int(size * 0.125)), operation='erosion')
                ], 
                p=0.5),
    ...
    valid_aug_list = A.Compose(transforms=[
        A.Resize(size, size),
        A.Normalize(
            mean = [0] * in_chans,
            std = [1] * in_chans
...
def read_image_mask(fragment_id, start_idx=17, end_idx=43):
    ...
    if CFG.scale_factor!=1: labels = cv2.resize(labels, (0, 0), fx = 1/CFG.scale_factor, fy = 1/CFG.scale_factor)

    fragment_mask = cv2.imread(CFG.comp_dataset_path + f"./segments/{fragment_id}/{fragment_id}_mask.png", 0)
    if CFG.scale_factor!=1: fragment_mask = cv2.resize(fragment_mask, (0, 0), fx = 1/CFG.scale_factor, fy = 1/CFG.scale_factor)  
    ...
    if CFG.reversed_load[fragment_id]:
      print("Reversed Load")
      idxs = range(64, 64-CFG.load_chans, -1)
    else:
    ...
    if CFG.TRAIN_ON_DIFF:
      ...
      for i in idxs:
        ...
        image = abs(image0 - image1p)
        image = image.astype(np.uint8)
        pad0 = (CFG.tile_size - image.shape[0] % CFG.tile_size)
        pad1 = (CFG.tile_size - image.shape[1] % CFG.tile_size)

        if CFG.scale_factor!=1: image = cv2.resize(image, (0, 0), fx = 1/CFG.scale_factor, fy = 1/CFG.scale_factor)
    ...
    _, labels = cv2.threshold(labels, 0, 255, cv2.THRESH_BINARY)
...
def get_train_valid_dataset(fragment_ids):
    ...
    for fragment_id in fragment_ids:
        ...
        image, label_mask, fragment_mask = read_image_mask(fragment_id, CFG.start_idxs[fragment_id], CFG.start_idxs[fragment_id] + (CFG.in_chans if fragment_id==CFG.valid_id else CFG.load_chans))
                ...
                for yi in range(0, CFG.tile_size, CFG.size):
                    for xi in range(0, CFG.tile_size, CFG.size):
                        ...
                        if fragment_id!=CFG.valid_id:
                            if not (np.all(label_mask[a:a + CFG.tile_size, b:b + CFG.tile_size]<0.5) or np.all(label_mask[a:a + CFG.tile_size, b:b + CFG.tile_size]>0.5)):
                                if not np.any(fragment_mask[a:a+ CFG.tile_size, b:b + CFG.tile_size]==0):
                                    train_images.append(image[y1:y2, x1:x2])
                                    train_masks.append(label_mask[y1:y2, x1:x2, None])
                                    assert image[y1:y2, x1:x2].shape==(CFG.size, CFG.size, CFG.load_chans)

                        if fragment_id==CFG.valid_id:
                            if (y1,y2,x1,x2) not in windows_dict:
                                if not np.any(fragment_mask[a:a + CFG.tile_size, b:b + CFG.tile_size]==0):
                                        valid_images.append(image[y1:y2, x1:x2])
                                        valid_masks.append(label_mask[y1:y2, x1:x2, None])
...
class CustomDataset(Dataset):
    ...
    def confused_tango(self,image):
        start_idx = random.randint(0, CFG.load_chans-CFG.in_chans)
        if random.random() > 0.5:
            image = image[..., start_idx:start_idx+CFG.in_chans]
        else:
            image = image[..., 0:CFG.in_chans]

        return image

    def fifth_augment(self,image):
    ...
    def fourth_augment(self,image):
        ...
        cropping_num = random.randint(self.cfg.in_chans//2, self.cfg.in_chans)
        ...
        if random.random() > 0.5: image_tmp[..., temporal_random_cutout_idx] = 0
        image = image_tmp
    ...
    def Prepare(self, image, label):
        ...
        label = data['mask']
        label = F.interpolate(label.unsqueeze(0),(self.cfg.size//(self.cfg.size//self.cfg.classes), self.cfg.size//(self.cfg.size//self.cfg.classes))).squeeze(0)
        return image, label
    ...
    def __getitem__(self, idx):
        ...
        if self.xyxys is not None:
            ...
            if self.transform:
                image, label = self.Prepare(image, label)
        ...
        ### Train
            ...
            label = self.labels[idx]
            if CFG.Sixth:
              image = self.confused_tango(image)

            if CFG.Fifth:
              image = self.fifth_augment(image)
...
class RegressionPLModel(pl.LightningModule):
    ...
    def configure_optimizers(self):
        optimizer = AdamW(self.parameters(), lr=CFG.lr)#torch.optim.RMSprop(self.parameters(), lr=CFG.lr, momentum=CFG.momentum)
...
# Check the state of major augs
if CFG.Fourth:print("Using Fourth")
if CFG.Fifth:print("Using Fifth")
if CFG.Sixth:
  print("Using Confused Tango!!")
else:
  CFG.load_chans = CFG.in_chans

valid_mask_gt = cv2.imread(CFG.comp_dataset_path + f"segments/{CFG.valid_id}/{CFG.valid_id}_T{CFG.exp_no}_inklabels.png", 0)
if CFG.scale_factor!=1: valid_mask_gt = cv2.resize(valid_mask_gt, (0, 0), fx = 1/CFG.scale_factor, fy = 1/CFG.scale_factor)  
...
#  model = RegressionPLModel.load_from_checkpoint(CFG.ckpt, strict=False, torch_dtype=torch.float16)
#else:
...
trainer = pl.Trainer(
    ...
    callbacks=[ModelCheckpoint(
      filename=f'{CFG.backbone}_T{CFG.trial_no}_E{CFG.exp_no}_size={CFG.size}_{CFG.valid_id}_'+'{epoch}',
...
trainer.fit(model=model, train_dataloaders=train_loader, val_dataloaders=valid_loader, ckpt_path=CFG.ckpt if CFG.ckpt else None)
