...
def Fifth_aug_old(img, strip_height=150):
...
class Fifth_aug(DualTransform):
    """Reverses the 3D volume upside down along the depth axis.

    Args:
        image: The 3D image to be reversed.

    Returns:
        The reversed 3D image.
    """
    ...
    def __init__(
        ...
        super(Fifth_aug, self).__init__(p, always_apply)

    def apply_with_params(self, old_image, **params):
        image = params['image'].copy()
        params['image'] = image[:,:,::-1]
        return params
...
class CFG:
    ...
    start_idxs =    {"S1_20230929220920":33,    "S4_20231210132040":17,      "S3_20240618142020":17,    "S3_20240716140050":17,    "S3_20240712074250":17,    "S1_20231004222109":33,      'S1_20230515162442':33,      '20230517214715':17,    '20230422011040':17,    '20230422213203':17,    '20230426114804':17,    '20230522182853':17,    '20230709155141':17,    '20230801194757':17,    '20240516205750':17,    '20230509173534':17}
    ...
    # ================== model cfg =================
    ckpt = "/content/drive/MyDrive/Scrolls/ckpts/Vesuvius_S2_T18/Training_S2_T18_E2/Vesuvius_S2_T18-models/weird_T18_E2_size=64_S1_20230515162442_epoch=3.ckpt"
    backbone = 'resnet3d'
    in_chans = 31
    classes = 4
...
class RegressionPLModel(pl.LightningModule):
    def __init__(self, pred_shape, size=256, with_norm=False, total_steps=780):
        ...
        self.hsdt = hsdt()
        #self.out = nn.Conv3d(CFG.in_chans, 1, 3, padding='same')
        ...
        self.decoder = Decoder(encoder_dims=[x.size(1) for x in self.backbone(torch.rand(1, 1, 20, CFG.size, CFG.size))], upscale=1)
...
trainer = pl.Trainer(
    ...
    callbacks=[ModelCheckpoint(filename=f'weird_T{CFG.trial_no}_E{CFG.exp_no}_size={CFG.size}_{CFG.valid_id}_'+'{epoch}', dirpath=CFG.model_dir, monitor='train/total_loss', mode='min', save_top_k=3)]
)
