...
class TimeSformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        for i in range(depth):
            ...
            if self.smoothed:
                gb = GaussianBlur(kernel_size=5, sigma=(sigma*(depth-i), sigma*(depth-i)))
                self.layers.append(nn.ModuleList([time_attn, spatial_attn, ff, gb]))
            else:
    ...
    def forward(self, video, mask = None):
        ...
        if self.smoothed:
            for (time_attn, spatial_attn, ff, gb) in self.layers:
                x = gb(x)
