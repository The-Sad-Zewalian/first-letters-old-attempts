...
from layers import Convx2, DownBlock, UpBlock, WithSE, ConvBLSTM, ConvLSTM
...
class HEDUNet(nn.Module):
    ...
    def __init__(self, input_channels, output_channels=2, base_channels=16,
                 ...
                 squeeze_excitation=False, lstm=False, merging='attention', stack_height=5,
        ...
        self.down_blocks = nn.ModuleList([
            DownBlock((1<<i)*bc, (2<<i)*bc, frame_size=(64>>(i+1),64>>(i+1)), **conv_args)
            for i in range(stack_height)
        ])

        self.up_blocks = nn.ModuleList([
            UpBlock((2<<i)*bc, (1<<i)*bc, frame_size=(64>>(i),64>>(i)), **conv_args)
