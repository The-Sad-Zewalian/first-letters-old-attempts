...
class HEDUNet(nn.Module):
    def __init__(self, input_channels, output_channels=2, base_channels=16,
                 ...
                 deep_supervision=True, smoothed_convs=False, sigma=0.1):
        super().__init__()
        bc = base_channels
        if squeeze_excitation:
            conv_block = WithSE(conv_block)

        self.init_gb = GaussianBlur(kernel_size=5, sigma=(sigma, sigma))
        ...
        self.gb_blocks_db = nn.ModuleList([
            GaussianBlur(kernel_size=5, sigma=(sigma*(i+1), sigma*(i+1)))
            for i in range(stack_height)
        ])
        ...
        self.gb_blocks_ub = nn.ModuleList([
            GaussianBlur(kernel_size=5, sigma=(sigma*(i+1), sigma*(i+1)))
            for i in reversed(range(stack_height))
        ])
        ...
        self.smoothed_convs = smoothed_convs
    ...
    def forward(self, x):
        ...
        if self.smoothed_convs:
          x = self.init_gb(x)
        x = self.init(x)

        skip_connections = []
        for block, gb_block in zip(self.down_blocks, self.gb_blocks_db):
            if self.smoothed_convs:
              x = gb_block(x)
        ...
        for block, skip, gb_block in zip(self.up_blocks, reversed(skip_connections), self.gb_blocks_ub):
            x = block(x, skip)
            if self.smoothed_convs:
              x = gb_block(x)
