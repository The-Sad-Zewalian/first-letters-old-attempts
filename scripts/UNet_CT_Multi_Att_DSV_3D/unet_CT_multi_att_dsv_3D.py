...
class unet_CT_multi_att_dsv_3D(nn.Module):
    ...
    def __init__(self, feature_scale=4, n_classes=21, is_deconv=True, in_channels=3,
                 nonlocal_mode='concatenation', attention_dsample=(1,2,2), is_batchnorm=True):
        ...
        self.conv5 = UnetConv3(filters[3], filters[4], self.is_batchnorm, kernel_size=(3,3,3), padding_size=(1,1,1))
        self.maxpool5 = nn.MaxPool3d(kernel_size=(1, 2, 2))

        self.center = UnetConv3(filters[4], filters[5], self.is_batchnorm, kernel_size=(3,3,3), padding_size=(1,1,1))
        self.gating = UnetGridGatingSignal3(filters[5], filters[5], kernel_size=(1, 1, 1), is_batchnorm=self.is_batchnorm)
        ...
        self.attentionblock4 = MultiAttentionBlock(in_size=filters[3], gate_size=filters[4], inter_size=filters[3],
                                                   nonlocal_mode=nonlocal_mode, sub_sample_factor= attention_dsample)
        self.attentionblock5 = MultiAttentionBlock(in_size=filters[4], gate_size=filters[5], inter_size=filters[4],
                                                   nonlocal_mode=nonlocal_mode, sub_sample_factor= attention_dsample)

        # upsampling
        self.up_concat5 = UnetUp3_CT(filters[5], filters[4], is_batchnorm)
        ...
        # deep supervision
        self.dsv5 = UnetDsv3(in_size=filters[4], out_size=n_classes, scale_factor=(1, 16, 16))
        self.dsv4 = UnetDsv3(in_size=filters[3], out_size=n_classes, scale_factor=(1, 8, 8))
        self.dsv3 = UnetDsv3(in_size=filters[2], out_size=n_classes, scale_factor=(1, 4, 4))
        self.dsv2 = UnetDsv3(in_size=filters[1], out_size=n_classes, scale_factor=(1, 2, 2))
    ...
    def forward(self, inputs):
        ...
        center = self.center(maxpool5)
        gating = self.gating(center)

        # Attention Mechanism
        # Upscaling Part (Decoder)
        g_conv5, att5 = self.attentionblock5(conv5, gating)
        up5 = self.up_concat5(g_conv5, center)

        g_conv4, att4 = self.attentionblock4(conv4, up5)
        ...
        final = torch.cat([dsv1,dsv2,dsv3,dsv4,dsv5], dim=1)
        #final = self.final(torch.cat([dsv1,dsv2,dsv3,dsv4], dim=1))#self.final(torch.cat([dsv1,dsv2,dsv3,dsv4,dsv5], dim=1))
