    ...
    def __getitem__(self, idx):
        if self.xyxys is not None:
            ...
            if self.transform:
                ...
                label = data['mask']
                label = F.interpolate(label.unsqueeze(0),(self.cfg.size//(self.cfg.size//4), self.cfg.size//(self.cfg.size//4))).squeeze(0)
...
class RCAN(nn.Module):
    def __init__(self, input_shape=(16, 256, 256, 1), num_channels=32, num_residual_blocks=5,
        ...
        self.output_conv = _conv(num_channels, num_output_channels, 3, padding=1)
...
class RegressionPLModel(pl.LightningModule):
    def __init__(self, pred_shape, size=256, with_norm=False, total_steps=780):
        ...
        #self.loss_func4 = nn.L1Loss()
        self.loss_func = lambda x,y: 0.5*self.loss_func1(x, y) + 0.5*self.loss_func2(x, y)# + 0.2*self.loss_func3(x, y)

        self.rcan = build_rcan(input_shape=(1, CFG.in_chans, CFG.size, CFG.size), num_channels=32, num_residual_blocks=5, num_residual_groups=5, channel_reduction=4, residual_scaling=1.0)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i, (x1, y1, x2, y2) in enumerate(xyxys):
            ...
            self.mask_pred[y1:y2, x1:x2] += F.interpolate(y_preds[i].unsqueeze(0).float(), scale_factor=CFG.size//4, mode='bilinear').squeeze(0).squeeze(0).numpy()
